...
export default function StudentScreen({ navigation }: any) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Avatar.Text size={80} label={(user?.FullName || 'U').slice(0,1)} style={{marginBottom:12}} />
      <Text style={styles.title}>{user?.FullName || user?.StudentName || 'Student'}</Text>
      <Text style={styles.info}>Email: {user?.Email || 'N/A'}</Text>
      <Text style={styles.info}>Mobile: {user?.MobileNo || 'N/A'}</Text>
      <Text style={styles.info}>ClassID: {user?.ClassID ?? 'N/A'}</Text>
      <Text style={styles.info}>URN: {user?.URNNO ?? 'N/A'}</Text>
      ...
      <Button mode="outlined" onPress={() => navigation.navigate('Debug')} style={{marginTop:10}}>
