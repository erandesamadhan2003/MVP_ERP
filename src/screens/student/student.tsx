    ...
    return (
      <View key={item.MenuIdentity}>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Icon source="folder-outline" size={20} color={level === 0 ? '#1649b2' : '#666'} />
          <Text style={[styles.menuItemText, level > 0 && styles.subMenuItemText]}>
            {item.MenuName}
          </Text>
          {hasChildren && (
            <Icon
        ...
        {isExpanded && hasChildren && (
          <View style={styles.submenuContainer}>
            {children.map((child: MenuItem) => (
              <MenuItem key={child.MenuIdentity} item={child} level={level + 1} />
            ))}
          </View>
        )}
      </View>
        ...
        <View style={styles.headerTop}>
          <View style={styles.userInfo}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={styles.userName}>{user?.FullName || user?.StudentName}</Text>
        ...
        <Card style={styles.menuCard}>
          <Card.Content style={styles.menuCardContent}>
            ...
            >
              ...
              <Icon
                source={isMenuExpanded ? 'chevron-up' : 'chevron-down'}
                size={24}
                color="#08306b"
              />
            </TouchableOpacity>
              ...
              <Divider style={{ marginVertical: 12 }} />
                {topLevelMenus.length > 0 ? (
                  topLevelMenus.map((item: MenuItem) => (
                    <MenuItem key={item.MenuIdentity} item={item} level={0} />
                  ))
                ) : (
          ...
          <Card.Content>
            ...
            <View style={styles.statsGrid}>
              ...
              <View style={styles.statGridItem}>
                <View style={[styles.statGridCard, { backgroundColor: '#fff0f2', borderLeftColor: '#ff4d6d' }]}>
                  <Avatar.Icon
                    size={48}
                    ...
                    color="#ff4d6d"
                  />
                  <Text style={styles.statGridCount}>{lecturesCount}</Text>
                  <Text style={styles.statGridLabel}>Lectures</Text>
                  <Button
                    mode="contained"
                    compact
              ...
              {/* Homework Card */}
              <View style={styles.statGridItem}>
                <View style={[styles.statGridCard, { backgroundColor: '#f0f4ff', borderLeftColor: '#4d79ff' }]}>
                  <Avatar.Icon
                    size={48}
                    icon="clipboard-text"
                    ...
                    color="#4d79ff"
                  />
                  <Text style={styles.statGridCount}>{homeworkCount}</Text>
                  <Text style={styles.statGridLabel}>Homework</Text>
                  <Button
                    mode="contained"
                    compact
              ...
              {/* Exams Card */}
              <View style={styles.statGridItem}>
                <View style={[styles.statGridCard, { backgroundColor: '#fff7ed', borderLeftColor: '#ff9f1a' }]}>
                  <Avatar.Icon
                    size={48}
                    ...
                    color="#ff9f1a"
                  />
                  <Text style={styles.statGridCount}>{examCount}</Text>
                  <Text style={styles.statGridLabel}>Exams</Text>
                  <Button
                    mode="contained"
                    compact
            ...
            <View style={styles.infoGrid}>
              <View style={styles.infoItem}>
                <Icon source="school" size={20} color="#1649b2" />
                <Text style={styles.infoLabel}>Class ID</Text>
                <Text style={styles.infoValue}>{user?.ClassID || 'N/A'}</Text>
              </View>
              <View style={styles.infoItem}>
                <Icon source="identifier" size={20} color="#4d79ff" />
                <Text style={styles.infoLabel}>URN No</Text>
                <Text style={styles.infoValue}>{user?.URNNO || 'N/A'}</Text>
              </View>
              <View style={styles.infoItem}>
                <Icon source="briefcase" size={20} color="#ff9f1a" />
                <Text style={styles.infoLabel}>SID</Text>
                <Text style={styles.infoValue}>{user?.SID || 'N/A'}</Text>
              </View>
              <View style={styles.infoItem}>
                <Icon source="domain" size={20} color="#ff4d6d" />
                <Text style={styles.infoLabel}>Org Code</Text>
                <Text style={styles.infoValue}>{user?.OCode || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.contactCard}>
              <View style={styles.contactRow}>
                <Icon source="email" size={18} color="#1649b2" />
                <Text style={styles.contactLabel}>Email:</Text>
                <Text style={styles.contactValue}>{user?.Email || 'N/A'}</Text>
              </View>
              <Divider style={{ marginVertical: 8 }} />
              <View style={styles.contactRow}>
                <Icon source="phone" size={18} color="#4d79ff" />
                <Text style={styles.contactLabel}>Mobile:</Text>
                <Text style={styles.contactValue}>{user?.MobileNo || 'N/A'}</Text>
              </View>
            </View>
          </Card.Content>
        </Card>
